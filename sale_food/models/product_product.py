...
from odoo.addons.sale_food import radar_template
...
class ProductProduct(models.Model):
    ...
    social_notation = fields.Selection(
        selection=_NOTATION_SELECTION,
        string="Social notation",
        required=True,
        default="0",
    )
    ...
    packaging_notation = fields.Selection(
        selection=_NOTATION_SELECTION,
        string="Packaging notation",
        required=True,
        default="0",
    )

    spider_chart_image = fields.Binary(
        compute="_compute_spider_chart_image",
        string="Spider Chart",
        store=True,
    )
    ...
    def _check_origin_department_country(self):
        for product in self:
            if (
                product.department_id.country_id
                and product.department_id.country_id.id
    ...
    def onchange_label_ids(self):
        for label in self.label_ids:
            self.social_notation = str(
                max(
                    int(self.social_notation),
                    int(label.minimum_social_notation),
                )
            )
            self.local_notation = str(
                max(
                    int(self.local_notation), int(label.minimum_local_notation)
                )
            )
            self.organic_notation = str(
                max(
                    int(self.organic_notation),
                    int(label.minimum_organic_notation),
                )
            )
            self.packaging_notation = str(
                max(
    ...
    def onchange_country_id(self):
        if self.country_id:
            ...
            if (
                self.department_id
                and self.department_id.country_id != self.country_id
            ):
    ...
    def onchange_state_id(self):
        if self.state_id:
            ...
            if (
                self.department_id
                and self.department_id.state_id != self.state_id
            ):
                self.department_id = False
            return {
                "domain": {
