...
class TestModule(TransactionCase):
    ...
    def test_01_generate_create_log(self):
        ...
        new_logs = self.ScaleLog.search([("id", "not in", self.initial_log_ids)])
        self.assertEqual(
            len(new_logs), 1, "set a scale group to a product should generate a new log"
        )
        new_log = new_logs[0]
        self.assertEqual(
            new_log.action, "create", "The log should be a 'creation' log."
        )
    ...
    def test_02_generate_delete_log(self):
        ...
        new_log = new_logs[0]
        self.assertEqual(
            new_log.action, "unlink", "The log should be a 'deletion' log."
        )
