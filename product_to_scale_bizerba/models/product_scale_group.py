...
class ProductScaleGroup(models.Model):
    ...
    screen_display = fields.Boolean(
        ...
        help="Check this box if you want" " to display the products on Customer Scale.",
    )

    screen_obsolete = fields.Boolean(
        string="Screen Obsolete",
        readonly=True,
        default=False,
    ...
    last_product_position = fields.Integer(
        compute="_compute_last_product_position", string="Last Product Position"
    )

    screen_product_qty = fields.Integer(
        string="Products quantity on Screen",
        help="Set the number of"
    ...
    scale_system_id = fields.Many2one(
        comodel_name="product.scale.system", string="Scale System", required=True
    )

    product_ids = fields.One2many(
        comodel_name="product.product", inverse_name="scale_group_id", string="Products"
    )
