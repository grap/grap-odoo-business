...
class ProductScaleSystem(models.Model):
    ...
    screen_text_file_pattern = fields.Char(
        string="Screen Text File Pattern",
        required=True,
        help="Pattern"
    ...
    field_ids = fields.One2many(
        comodel_name="ir.model.fields", string="Fields", compute="_compute_field_ids"
    )

    product_id_field_id = fields.Many2one(
        comodel_name="ir.model.fields",
        string="Product ID Field",
    ...
    def _compute_field_ids(self):
        for system in self:
            system.field_ids = (
                system.product_line_ids.filtered(lambda x: x.field_id)
                .mapped("field_id")
                .ids
            )
