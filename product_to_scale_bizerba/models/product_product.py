...
class ProductProduct(models.Model):
    ...
    def _compute_external_id_bizerba(self):
        for product in self:
            if product.scale_group_id:
                ...
                if product_id_field:
                    product.external_id_bizerba = getattr(
                        product, product_id_field.name
                    )
    ...
    def send_scale_create(self):
        for product in self:
            product._send_to_scale_bizerba(
                "create", product.scale_group_id, product.scale_group_id.scale_system_id
            )
    ...
    def send_scale_write(self):
        for product in self:
            product._send_to_scale_bizerba(
                "write", product.scale_group_id, product.scale_group_id.scale_system_id
            )
    ...
    def send_scale_unlink(self):
        for product in self:
            product._send_to_scale_bizerba(
                "unlink", product.scale_group_id, product.scale_group_id.scale_system_id
            )
    ...
    def _send_to_scale_bizerba(self, action, scale_group, scale_system):
        ...
        if not product.external_id_bizerba:
            raise UserError(
                _(
                    ...
                    % (scale_system.product_id_field_id.name, product.name)
                )
            )
        ...
        if (
            product.scale_group_id.screen_display
            and not product.scale_group_id.screen_obsolete
        ):
    ...
    def write(self, vals):
        ...
        for product_id, action in defered.items():
            ...
            product._send_to_scale_bizerba(
                action, product.scale_group_id, product.scale_group_id.scale_system_id
            )
